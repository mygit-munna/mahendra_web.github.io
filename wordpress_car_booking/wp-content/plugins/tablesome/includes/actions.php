...
if ( !class_exists( '\\Tablesome\\Includes\\Actions' ) ) {
    class Actions
    {
        ...
        public function create_new_email_logs_trigger_table()
        {
            ...
            if ( $table_id ) {
                $onboarding_page_url = add_query_arg( 'link_action', 'EMAIL_LOGS', $onboarding_page_url );
                $onboarding_page_url = add_query_arg( 'status', 'CREATED_EMAIL_LOGS_TRIGGER_TABLE', $onboarding_page_url );
            } else {
                // redirect to create new table page
                $onboarding_page_url = add_query_arg( 'link_action', 'EMAIL_LOGS', $onboarding_page_url );
                $onboarding_page_url = add_query_arg( 'status', 'FAILED_TO_CREATE_EMAIL_LOGS_TRIGGER_TABLE', $onboarding_page_url );
            }
        ...
        public function publish_table()
        {
            ...
            if ( empty($table) ) {
                // add error message
                $onboarding_page_url = add_query_arg( 'link_action', 'EMAIL_LOGS', $onboarding_page_url );
                $onboarding_page_url = add_query_arg( 'status', 'INVALID_POST_ID', $onboarding_page_url );
                wp_redirect( $onboarding_page_url );
                return;
            }
            ...
            $onboarding_page_url = add_query_arg( 'link_action', 'EMAIL_LOGS', $onboarding_page_url );
            $onboarding_page_url = add_query_arg( 'status', 'PUBLISHED_TABLE', $onboarding_page_url );
            // reload page
            wp_redirect( $onboarding_page_url );
            exit;
        }
